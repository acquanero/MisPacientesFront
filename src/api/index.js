...
API.interceptors.response.use(
	async (response) => {
		DEBUG && console.log(`${response.status} - ${getUrl(response.config)}:`, response.data);
		return response;
	},

	(error) => {
		DEBUG && console.log(`${error.response.status} - ${getUrl(error.response.config)}:`, error.response);
		return Promise.reject(error);
	}
);
